...
describe("The `RecipeForm` component", () => {
  ...
  const getPrefFields = (forms) => {
    const actionArgumentsForm = findForm(forms, "Action Arguments");
    const actionArgumentGroups = within(actionArgumentsForm).getAllByRole(
      "group",
    );
    const experimentSlugForm = findForm(
      actionArgumentGroups,
      "Experiment Slug",
    );
    const preferenceNameForm = findForm(
      actionArgumentGroups,
      "Preference Name",
    );
    const experimentDocURLForm = findForm(
      actionArgumentGroups,
    ...
    const comboBoxes = within(actionArgumentsForm).getAllByRole("combobox");
    const prefTypeField = comboBoxes[0];
    const prefBranchTypeField = comboBoxes[1];
    const buttons = within(actionArgumentsForm).getAllByRole("button");
    const highVolumeField = buttons[0];
    const preventNewEnrollField = buttons[1];
    const addBranchButton = buttons[2];
